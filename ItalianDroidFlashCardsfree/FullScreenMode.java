...
public class FullScreenMode extends mainActivity implements OnTouchListener,
...
//		});
		...
		pronounce.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (currentQuestionNumber % 2 != 0) {
					...
					result = mTts.setLanguage(Locale.ITALIAN);//change here
		...
		mQuestions = new Hashtable<Integer, Question>(CARD_BATCH_SIZE);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.englishmode_false:
			englishmode = false;
			Toast.makeText(this, "Italian Deck Face Up", Toast.LENGTH_SHORT)//change here
					.show();
			if (currentQuestionNumber % 2 != 0) {
				flipToFrench(true, true);
			}
		...
		case R.id.speak:
			...
			return true;
		case R.id.use_dict:
			startActivity(new Intent(FullScreenMode.this, Dictionary.class));
			return true;
		case R.id.help:
			TextView msg = new TextView(this);
			msg.setText("Speak Icon: Pronounces the word for you. \nShuffle: Rearranges the card deck order. \nFlip to Deck: Sets the first card to view when switching to a new card. \nDictionary: Look up a word with an internet enabled dictionary. \nQuit: Closes the entire application");
	...
	public void onInit(int status) {
		...
		if (status == TextToSpeech.SUCCESS) {
			...
			if (result == TextToSpeech.LANG_MISSING_DATA
				...
				// sayHello();
				TextView msg = new TextView(this);
				msg.setText("You do not have text to speech installed on your phone");
				msg.setGravity(Gravity.CENTER_HORIZONTAL);

				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setView(msg)
						.setCancelable(true)
						.setPositiveButton("Get It Now For Free",
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
											int id) {
	...
	private void loadQuestionBatch(int number) throws XmlPullParserException,
		...
		case 1:
			questionBatch = getResources().getXml(R.xml.animals);
			CARD_BATCH_SIZE = 102;
			break;

		case 2:
			questionBatch = getResources().getXml(R.xml.body);
			CARD_BATCH_SIZE = 138;
			break;
		case 3:
			questionBatch = getResources().getXml(R.xml.colors);
			CARD_BATCH_SIZE = 30;
			break;

		case 4:
			questionBatch = getResources().getXml(R.xml.conjugation);
			CARD_BATCH_SIZE = 72;
			break;
		case 5:
			questionBatch = getResources().getXml(R.xml.descriptions);
			CARD_BATCH_SIZE = 78;
			break;
		case 6:
			questionBatch = getResources().getXml(R.xml.family);
			CARD_BATCH_SIZE = 80;
			break;
		case 7:
			questionBatch = getResources().getXml(R.xml.foods);
			CARD_BATCH_SIZE = 58;
			break;

		case 8:
			questionBatch = getResources().getXml(R.xml.fruits_and_vegetables);
			CARD_BATCH_SIZE = 64;
			break;
		case 9:
			questionBatch = getResources().getXml(R.xml.greetings);
			CARD_BATCH_SIZE = 178;
			break;

		case 10:
			questionBatch = getResources().getXml(R.xml.in_the_city);
			CARD_BATCH_SIZE = 112;
			break;

		case 11:
			questionBatch = getResources().getXml(R.xml.months_and_seasons);
			CARD_BATCH_SIZE = 32;
			break;

		case 12:
			questionBatch = getResources().getXml(R.xml.numbers);
			CARD_BATCH_SIZE = 118;
			break;

		case 13:
			questionBatch = getResources().getXml(R.xml.traveling);
			CARD_BATCH_SIZE = 88;
			break;

		case 14:
			questionBatch = getResources().getXml(R.xml.weather);
			CARD_BATCH_SIZE = 64;
			break;
		default:
			questionBatch = getResources().getXml(R.xml.errordeck);
			CARD_BATCH_SIZE = 2;
			break;
		}
		// END MOCK QUESTIONS
		...
		if (initialBatchLoad == true) {
			...
			if (currentQuestionNumber == 0) {
				currentQuestionNumber = CARD_BATCH_SIZE;
			}
			if (currentQuestionNumber == -1) {
				currentQuestionNumber = CARD_BATCH_SIZE - 1;
			}
			if (currentQuestionNumber == -2) {
				currentQuestionNumber = CARD_BATCH_SIZE - 2;
			}
			if (currentQuestionNumber == CARD_BATCH_SIZE + 1) {
				currentQuestionNumber = 1;
			}
			if (currentQuestionNumber == CARD_BATCH_SIZE + 2) {
				currentQuestionNumber = 2;
			}
			if (currentQuestionNumber == CARD_BATCH_SIZE + 3) {
				currentQuestionNumber = 3;
			}
		}
		// Parse the XML
		...
		randomQuestionNum = new Integer[CARD_BATCH_SIZE];
		questionNum = new Integer[CARD_BATCH_SIZE];
		questionText = new String[CARD_BATCH_SIZE];
		...
		if (shuffleBoolean == false) {
			for (int i = 0; i < CARD_BATCH_SIZE; i++) {
			...
			for (int i = 0; i < CARD_BATCH_SIZE; i++) {
				if (i % 2 == 1) {
					...
					Random r = new Random();
					int randomNumber = r.nextInt(CARD_BATCH_SIZE);
					...
					for (int j = 1; j <= CARD_BATCH_SIZE; j++) {
						if (randomQuestionNum[j] == null) {
							break;
						}
						if (randomQuestionNum[j] == randomNumber) {
							randomNumber += 2;
							if (randomNumber > CARD_BATCH_SIZE) {
								randomNumber = 2;
	...
	private void loadCurrentDeck(int number) {
		...
		if (number < 1) {
			do {
				CURRENT_DECK -= 1;
				if (CURRENT_DECK == 0) {
					CURRENT_DECK = NUMBER_OF_DECKS;
				}
			} while (cardDeckChecked[CURRENT_DECK - 1] == false);
		}
		if (number > CARD_BATCH_SIZE) {
			do {
				CURRENT_DECK += 1;
				if (CURRENT_DECK == NUMBER_OF_DECKS + 1) {
					CURRENT_DECK = 1;
				}
			} while (cardDeckChecked[CURRENT_DECK - 1] == false);
		}
